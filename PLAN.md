...
- **Trigger**: euclidean distance from the host's centre to the other robot's centre `<= props.threshold` (default 260). Chosen over the vehicle-detection cone for a symmetric, cheap "physical spread" model that makes the target selector a clean pure function.
